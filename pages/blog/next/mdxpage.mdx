<div className='mdx padding'>


<div className='center margin xxxlarge'>How to write html elements</div>
...
```
<p><a href="https://mdxjs.com" title="title">HTML</a></p>
or
<a href="https://mdxjs.com" title="title">HTML</a>
```
...
```
<p><img src="/favicon.ico" alt="Alt text" title="title" /></p>
```
...
<div className='center margin xxxlarge'>how to use markdown (MDX)</div>
...
\
[For more information](https://spec.commonmark.org/0.30/ "mdx")


<div className="mdx rtl">

# استخدام `MDX` مع `next.js` :

* `mdx` عبارة عن مجموعة شاملة من عمليات `markdown` يتيح لك الكتابة `JSX`  مباشرة في الملفات `markdown` الخاص بك .

* إنها طريقة قوية لإضافة تفاعل ديناميكي وتضمين المكونات في المحتوى الخاص بك حيث تساعدك على إضفاء الحيوية على صفحاتك .

* `Next.js` يدعم `MDX` من خلال عدد من الوسائل المختلفة ستوضح هذه الصفحة بعض الطرق التي يمكنك من خلالها البدء بدمج `mdx` في مشروع `next.js` الخاص بك .

## لماذا استخدم `MDX` ؟

* التأليف في `markdown` هو طريقة بديهية لكتابة المحتوى ، وبمجرد اعتماده ، يمكن أن يمكّنك من كتابة محتوى يمكن قراءته وصيانته. نظرًا لأنه يمكنك استخدام عناصر `HTML` في عملية الشطب ، يمكنك أيضًا أن تكون مبدعًا عند تصميم صفحات `markdown` الخاصة بك .

* ومع ذلك ، نظرًا لأن `markdown` هو في الأساس محتوى ثابت ، فلا يمكنك إنشاء محتوى ديناميكي بناءً على تفاعل المستخدم. حيث تتألق `MDX` في قدرتها على السماح لك بإنشاء مكونات `React` واستخدامها مباشرة في الترميز . يفتح هذا مجموعة واسعة من الاحتمالات عند إنشاء صفحات مواقعك مع وضع التفاعل في الاعتبار .
